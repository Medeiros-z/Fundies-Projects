...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname Pipe_Fantasy_v3) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
;; (pipe-fantasy (make-gamestate DEMO-GRID DEMO-GOO-FLOW DEMO-PIPES 15 100 30 140))
...
(define DEMO-PIPES (list (make-pipe #false #true #false #true)
...
;; pipe-image: pipe Number Number boolean boolean boolean -> image
...
(define (pipe-image pipe tile-size pipe-width filled? vertical? fill-all?)
  (local
    [(define (pipe-image/custom-color pipe tile-size pipe-width bg-color pipe-color)
       (local
         [(define mid-left-value (quotient (- tile-size pipe-width) 2))
         ...
         (foldl
          ...
          pipe-offsets-y
          pipe-sub-imgs)))]
    (if (and filled? (pipe-top pipe) (pipe-bottom pipe) (pipe-right pipe) (pipe-left pipe))
        (if fill-all?
            (pipe-image/custom-color pipe tile-size pipe-width "grey" "green")
            (overlay
             (if vertical?
                 (pipe-image/custom-color (make-pipe #t #t #f #f)
                                          tile-size pipe-width "transparent" "green")
                 (pipe-image/custom-color (make-pipe #f #f #t #t)
                                          tile-size pipe-width "transparent" "green"))
             (pipe-image/custom-color pipe tile-size pipe-width "grey" "black")))
        (pipe-image/custom-color pipe tile-size pipe-width "grey" (if filled? "green" "black")))))
...
(define DEMO-GRID (make-grid 6 (list (list 1 1 (make-pipe #false #false #false #true))
...
(define DEMO-GOO-FLOW (list (list 1 1 "right")))
                      ...
                      (my-member? (list x y)
                                  (map (lambda (elem) (list (first elem) (second elem)))
                                       (if (and (not (empty? goo-flow))
                                           ...
                                           (rest goo-flow)
                                           goo-flow)))
                      (ormap (lambda (gf) (and (cons? gf)
                                               (= x (first gf))
                                               (= y (second gf))
                                               (or (string=? "top" (third gf))
                                                   (string=? "bottom" (third gf)))))
                             goo-flow)
                      (< 1 (length (filter (lambda (gf) (and (cons? gf)
                                                             (= x (first gf))
                                                             (= y (second gf))))
                                           goo-flow))))
          img-x
...
(define-struct gamestate [grid goo-flow incoming-pipes num-of-pipes tile-size pipe-width next-goo])
;; A gamestate is
...
;; num-of-pipes: number of pipes in the game (i.e. starting value of incoming-pipes)
;; tile-size: size of the tile in the interface.
;; pipe-width: width of the pipe in the interface.
;; next-goo: time until next goo propagation
...
(define (get-score gs)
  (if (false? (first (gamestate-goo-flow gs)))
      (* 50 (+ (- (- (length (gamestate-goo-flow gs)) 2)
                  (- (gamestate-num-of-pipes gs) (length (grid-cells (gamestate-grid gs))))) (length (gamestate-incoming-pipes gs))))
      (* 50 (+ (- (- (length (gamestate-goo-flow gs)) 1)
                  (- (gamestate-num-of-pipes gs) (length (grid-cells (gamestate-grid gs))))) (length (gamestate-incoming-pipes gs))))))

(check-expect (get-score (make-gamestate DEMO-GRID DEMO-GOO-FLOW DEMO-PIPES 15 100 30 140)) 0)
(check-expect (get-score (make-gamestate (make-grid 6 (list (list 1 1 (make-pipe #false #false #true #false))))
                                         (list (list 0 1 "left") (list 1 1 "left")) PASS-THROUGH-PIPES 8 100 30 28)) 50)
(check-expect (get-score (make-gamestate (make-grid 7 (list (list 1 1 (make-pipe #true #false #false #false))))
                                         (list #false (list 1 1 "top")) PASS-THROUGH-PIPES 8 100 30 28)) 0)
  ...
  (local
    [(define incoming-pipes (gamestate-incoming-pipes gs))
     ...
     (define incoming-pipes-img
       (above
        (text (string-append "Current Score:" "\n"
                             (number->string (get-score gs))) 20 "grey")
        (above
         (text "NEXT PIPE" 20 "grey")
         (foldr
          (lambda (pipe img)
            (above (pipe-image pipe tile-size pipe-width #false #false #false)
                   img))
          empty-image
...
(define (gamestate-propagate-goo gs)
  (make-gamestate (gamestate-grid gs)
                  ...
                  (gamestate-num-of-pipes gs)
                  (gamestate-tile-size gs)
                  (gamestate-pipe-width gs)
                  (gamestate-next-goo gs)))
...
(define (gamestate-place-pipe-on-click gs x y event)
  (if (false? (first (gamestate-goo-flow gs)))
      ...
      (local
        ...
        (if (ormap (lambda (gf) (and (cons? gf)
                                     (= grid-x (first gf))
                                     (= grid-y (second gf))))
                   (gamestate-goo-flow gs))
            gs
...
;; move-goo : gamestate -> gamestate
;; Automatically moves the goo through the pipes
(define (move-goo gs)
  (cond
    [(not (= 0 (gamestate-next-goo gs)))
     (make-gamestate (gamestate-grid gs)
                     (gamestate-goo-flow gs)
                     (gamestate-incoming-pipes gs)
                     (gamestate-num-of-pipes gs)
                     (gamestate-tile-size gs)
                     (gamestate-pipe-width gs)
                     (- (gamestate-next-goo gs) 1))]
    [(= 0 (gamestate-next-goo gs))
     (make-gamestate (gamestate-grid gs)
                     (grid-goo-propagate
                      (gamestate-grid gs)
                      (gamestate-goo-flow gs))
